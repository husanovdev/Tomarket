        ...
        while True:
            try:
                                ...
                                if play_game.get('status') == 0:
                                    ...
                                    if claim_game and 'status' in claim_game:
                                        ...
                                        if claim_game.get('status') == 0:
                                            ...
                                            logger.info(
                                                f"{self.session_name} | Claimed points: {claim_game.get('data').get('points')}, Tickets left: {tickets}")
                                            await asyncio.sleep(1.5)
                        logger.info(
                ...
                if settings.AUTO_TASK:
                    ...
                    if tasks and tasks.get("status", 500) == 0:
                        ...
                        for category, task_group in tasks["data"].items():
                            if isinstance(task_group,
                                          list):  # Ensure task_group is a list (e.g., "standard", "chain_free")
                                for task in task_group:
                                    if isinstance(task, dict):  # Ensure each task is a dictionary
                                        if task.get('enable') and not task.get('invisible', False) and task.get(
                                                'status') != 3:
                                            ...
                                            if task.get('startTime') and task.get('endTime'):
                                                ...
                                                if task_start <= time() <= task_end and task.get('type') not in [
                                                    'charge_stars_season2', 'chain_donate_free', 'daily_donate',
                                                    'new_package']:
                                            ...
                                            # For tasks that do not require time validation
                                                                          ...
                                                                          'charge_stars_season2', 'chain_donate_free',
                                                                          'daily_donate']:
                                                tasks_list.append(task)
                    ...
                    for task in tasks_list:
                        ...
                        if task.get('type') == 'free_tomato':
                            logger.info(
                                f"{self.session_name} | Start task <light-red>{task['name']}.</light-red> Wait {3}s 🍅")
                            await asyncio.sleep(3)
                            await self.check_task(http_client=http_client,
                            ...
                            if settings.AUTO_CHANGE_NAME:
                                logger.info(
                                    f"{self.session_name} | Start task <light-red>{task['name']}.</light-red> Wait {30}s 🍅")
                                await asyncio.sleep(30)
                                ...
                                starttask = await self.start_task(http_client=http_client,
                                                                  data={'task_id': task['taskId'],
                                                                        'init_data': init_data})
                                await asyncio.sleep(3)
                                check = await self.check_task(http_client=http_client,
                                                              data={'task_id': task['taskId'], 'init_data': init_data})
                                await asyncio.sleep(3)
                                if check:
                                    logger.info(
                                        f"{self.session_name} | Task <light-red>{task['name']}</light-red> checked! 🍅")
                    ...
                    if combo_info.get('status') == 0 and combo_info_data is not None:
                        if combo_info_data.get('status') > 0:
                            logger.info(f"{self.session_name} | Combo already claimed | Skipping....")
                                ...
                                combo_info_data.get('endTime')) > datetime.now():
                            ...
                            if payload is None:
                                logger.error(
                                    f"{self.session_name} | Failed to retrieve puzzle payload | Payload: {payload}")
                                return
                            ...
                            if claim_combo is not None and claim_combo.get('status') == 0:
                                ...
                                logger.info(
                                    f"{self.session_name} | Combo not claimed. Reason: {claim_combo.get('message', 'Unknown error')}")
                ...
                if settings.AUTO_RAFFLE:
                    tickets = await self.get_ticket(http_client=http_client,
                                                    data={"language_code": "en", "init_data": init_data})
                            ...
                            while tickets > 0:
                                ...
                                if play_ticket and play_ticket.get('status', 500) == 0:
                                    ...
                                    if results:
                                        ...
                                        logger.info(
                                            f"{self.session_name} | Raffle result: {amount} | <light-red>{item_type}</light-red>")
                                    tickets -= 1
                                    await asyncio.sleep(5)
